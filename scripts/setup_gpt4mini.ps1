...
try {
    $openaiInstalled = pip show openai 2>&1
...
"@
    ...
    Write-Host "  ✗ Error verifying API key: $_" -ForegroundColor Red
    exit 1
}
...
if (-not $SkipTest) {
    Write-Host "`n[6/6] Running test extraction ($TestSample documents)..." -ForegroundColor Yellow
    ...
    try {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Host "  ⚠ Test extraction failed" -ForegroundColor Yellow
        }
    } catch {
        Write-Host "  ⚠ Error during test extraction: $_" -ForegroundColor Yellow
    }
} else {
    Write-Host "`n[6/6] Skipping test extraction (--SkipTest)" -ForegroundColor Yellow
}
...
Write-Host "     robimb extract properties --input data.jsonl --output results.jsonl \" -ForegroundColor Gray
Write-Host "       --llm-endpoint http://localhost:8000/extract --llm-model gpt-4o-mini \" -ForegroundColor Gray
Write-Host "       --max-workers 8 --sample 50" -ForegroundColor Gray
Write-Host ""
Write-Host "  2. View stats:" -ForegroundColor White
Write-Host "     curl http://localhost:8000/stats" -ForegroundColor Gray
Write-Host ""
