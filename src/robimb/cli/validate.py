...
def main(argv: List[str] | None = None) -> None:
    ...
    if hasattr(config, "label_texts_super"):
        model = load_label_embed_model(model_dir, backbone_src=getattr(config, "backbone_src", None), tokenizer=tokenizer)
    else:
        model = load_masked_model(model_dir)

    dataset = _build_dataset(args.test_file, args.max_length, tokenizer)
    ...
    predictions_super = []
    predictions_cat_pred = []
    predictions_cat_gold = []
    labels_super = []
    labels_cat = []
    ...
    labels_super = np.concatenate(labels_super)
    labels_cat = np.concatenate(labels_cat)
            ...
            preds.append(
                {
                    ...
                    "super_gold": s_id2name[int(labels_super[idx])],
                    "cat_gold": c_id2name[int(labels_cat[idx])],
                }
            )
